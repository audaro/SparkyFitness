...
| **Manage Check-in**<br>`can_manage_checkin` | ✍️ Weight, body measurements, progress photos and the measurements taken from them, mood logs, fasting status, sleep logs, smart scale composition (muscle mass, bone mass, water %, BMR), and generic health telemetry (heart rate, HRV, respiration, SpO2, vitals, daily health scores, and captured wearable calorie totals). | 📖 Weight history, progress photos and their before/after comparisons, wellness logs (mood, fasting, sleep), smart scale metrics, generic health telemetry, and read-only profile/layout settings.<br><br>❌ *Diary logs (food, exercise, water) and medication logs are completely blocked.* |
...
* AI Coach profile (`coach_profiles` table — coaching goals, training constraints, experience level, injuries and limitations, food preferences, weekly set targets, and the training-plan questionnaire's answers including any stated testosterone use). This coaching context is not shareable and is never family-delegated. The training-plan answers are owner-only for the same reason as the rest of the table, and the stated-testosterone field is additionally kept out of everything sent to the AI chat provider.
* Gym equipment profiles (`gym_equipment_profiles` table — the named equipment sets you switch between, such as "Home" or your gym). These name the real places you train, ride the same coaching context, and are not shareable.
* Your generated "Up Next" workout (`workout_recommendations` table — the suggested workout built from your own recovery and training history). Built from that same coaching context, so it is not shareable.
