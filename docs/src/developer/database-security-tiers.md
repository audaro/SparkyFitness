...
| `coach_profiles` | AI coach context (goals, training constraints, experience level, injuries/limitations, food preferences, personal aliases, weekly set targets) plus the training-plan answers (primary goal, physique target, priority muscle groups, **enhancement**, plan completion). `enhancement` records exogenous testosterone and is the most sensitive column on the table: owner-only by RLS, never family-delegated, and deliberately excluded from everything sent to the AI chat provider. The one thing that reads it is the lean-mass projection, which is computed on the server and returns kilograms and a boolean — the stated dose is not part of that response either. | Owner-Only | Owner-Only |
| `gym_equipment_profiles` | Named, switchable gym equipment sets ("Home", "Planet Fitness"); the active one constrains workout generation and catalog search | Owner-Only | Owner-Only |
| `workout_recommendations` | The generated "Up Next" workout, derived from the user's own recovery and training history | Owner-Only | Owner-Only |
...
| `user_meal_visibilities` | Which meal types are shown in the diary | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `user_custom_nutrients` | Custom nutrient definitions | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `user_allergen_preferences` | Allergen preferences applied to food search and warnings | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `goal_presets` | Named calorie/macro/water goal sets the user switches between | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `user_goals` | Active daily calorie/macro goals | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `user_nutrient_goal_preferences` | Per-user minimum/maximum/target goal direction override per nutrient | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `weekly_goal_plans` | Assigns a goal preset to each weekday over a date range | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `user_water_containers` | Configured container sizes (e.g. 500ml bottle), hydration factors, and optional links to food/variant/meal category | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
...
| `meal_plans` | Weekly meal planning schedules | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
...
| `check_in_photos` | Progress photos (image bytes served only via the authenticated `GET /api/measurements/check-in-photos/file/{id}`; excluded from the public `/uploads` static mount). The row also carries `capture_meta`: how the photo was taken - capture mode, device model, camera facing, the photo it was framed against, and the local time of day. It rides the same permissions as the image, since it describes it | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
| `check_in_photo_analysis` | Body measurements taken from one progress photo by the vision service (silhouette widths, scale-free ratios, pose landmarks). Derived data only, safe to delete and rebuild; it rides the photo's permissions because it describes the photo | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
| `progress_photo_comparisons` | One cached before/after pair: both photos' measurements, how well they aligned, and how much the lighting changed. No `verdict` column - whether a pair may honestly be compared is recomputed from these numbers on every read, so tuning the thresholds reaches pairs measured months earlier | Delegate with `can_manage_checkin` | Delegate with `can_manage_checkin` or `can_view_reports` |
