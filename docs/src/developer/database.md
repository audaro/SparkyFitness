...
| `passkey_registration_tickets` | Single-use tickets for enrolling a passkey from a second device (hashed, expiring) |
| `user_oidc_links` | Links a local account to an external OIDC provider subject |
...
| `food_favorites` | Foods and meals the user starred; exactly one of `food_id`/`meal_id` is set |
...
| `coach_profiles` | Training and food context behind generation and AI chat — goals, days per week, session length, limitations, weekly set targets, experience level, and the training-plan answers (primary goal, physique target, priority muscle groups, enhancement, plan completion). Owner-only (Tier 1) |
...
| `check_in_photo_analysis` | Cached body measurements for one progress photo |
| `progress_photo_comparisons` | Cached before/after measurements for a pair of progress photos |
...
| `user_custom_symptom_locations` | User-defined body locations a symptom can be logged against |
...
| `goal_presets` | Named calorie/macro/water goal sets the user can switch between |
| `weekly_goal_plans` | Assigns a goal preset to each weekday over a date range; one active plan per user |
