...
Nine defects were found and fixed on the way, all of them real rather than test
...
errors; five accessibility collapses (`AddSheet` / `ActionSheet`, the active
workout's set row, `ActivityAddScreen`'s form wrapper, `AnchoredMenu`'s
dismissal backdrop wrapping the menu it dismisses, and finally the fifteen
remaining bottom sheets that had never been opted out at all) that hid whole
subtrees from VoiceOver as much as from the driver; and — found by building the duration
