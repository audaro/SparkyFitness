...
describe('language picker excludes unshipped Weblate locales', () => {
  it('SUPPORTED_LANGUAGES contains only shipped locales (en, pl, es, de) and not unshipped (fr)', () => {
    // The production registry ships en + pl + es + de. Even if Weblate syncs
    // other catalogs, those locales must not appear in the runtime language picker.
  ...
  it('SHIPPED_LOCALES includes de but not fr', () => {
    expect(Object.keys(SHIPPED_LOCALES)).toContain('de');
...
describe('DE shipped → generated RESOURCE_MAP includes DE', () => {
  ...
  it('DE shipped → production RESOURCE_MAP includes DE', () => {
    expect(Object.keys(RESOURCE_MAP)).toContain('de');
  });
});
