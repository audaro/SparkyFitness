...
| **Workouts** (presets/plans) | `routes/workoutPresetRoutes.ts` `routes/workoutPlanTemplateRoutes.ts`, `services/workoutPresetService.ts` `services/workoutPlanTemplateService.ts`, `models/workoutPresetRepository.ts` `models/workoutPlanTemplateRepository.ts` | `pages/Exercises/` | `screens/Workout*Screen.tsx` `services/api/workoutPresetsApi.ts` | `schemas/database/ExercisePresetEntries.zod.ts` `WorkoutPresets.zod.ts` `WorkoutPresetExercises.zod.ts` `WorkoutPresetExerciseSets.zod.ts` `WorkoutPlanTemplates.zod.ts` `WorkoutPlanTemplateAssignments.zod.ts` `WorkoutPlanAssignmentSets.zod.ts` `WorkoutRecommendations.zod.ts`, `schemas/api/WorkoutPresets.api.zod.ts` `WorkoutRecommendations.api.zod.ts` |
...
| **Coaching & Recommendations** (gym profiles, coach profile, weekly set targets, muscle recovery) | `routes/coachProfileRoutes.ts` `routes/gymEquipmentProfileRoutes.ts` `routes/weeklySetTargetRoutes.ts` `routes/weeklyGoalPlanRoutes.ts` `routes/workoutRecommendationRoutes.ts`, `services/workoutRecommendationService.ts` `services/weeklySetTargetService.ts` `services/weeklyGoalPlanService.ts`, `models/coachProfileRepository.ts` `models/coachRepository.ts` `models/gymEquipmentProfileRepository.ts` `models/workoutRecommendationRepository.ts` `models/weeklyGoalPlanRepository.ts` | `pages/Exercises/GymProfilesManager.tsx` `pages/Exercises/MuscleRecoveryCard.tsx` `pages/Reports/MuscleGroupRecoveryTracker.tsx`, `api/Exercises/coachProfile.ts` `gymProfiles.ts` `workoutRecommendations.ts`, `hooks/Exercises/useCoachProfile.ts` `useGymProfiles.ts` `useMuscleRecovery.ts` `useWorkoutRecommendation.ts` `useCoachingContextAvailable.ts` | `screens/GymProfilesScreen.tsx` `screens/PickMusclesScreen.tsx` `screens/WeeklySetTargetsScreen.tsx` `screens/OnDemandWorkoutsScreen.tsx`, `hooks/useCoachProfile.ts` `useGymProfiles.ts` `useMuscleRecovery.ts` `useWeeklySetTargets.ts` `useWorkoutRecommendation.ts` `useGenerateAndShowWorkout.ts`, `services/api/coachProfileApi.ts` `gymProfilesApi.ts` `weeklySetTargetsApi.ts` `workoutRecommendationsApi.ts` | `utils/muscleRecovery.ts` `utils/muscleRecoveryDisplay.ts` `utils/weeklySetTargets.ts`, `schemas/database/CoachProfiles.zod.ts` `GymEquipmentProfiles.zod.ts` `WorkoutRecommendations.zod.ts`, `schemas/api/CoachProfile.api.zod.ts` `GymEquipmentProfiles.api.zod.ts` `WorkoutRecommendations.api.zod.ts` (tables `coach_profiles`, `gym_equipment_profiles`, `weekly_goal_plans`, `workout_recommendations`) |
