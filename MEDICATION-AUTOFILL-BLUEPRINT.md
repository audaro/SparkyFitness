...
### Curated dosage ladders (new data, small)

`glp1.ts` has PK constants but no strengths. Add a `strengths` field per profile — these are
titration ladders, which for GLP-1s is genuinely what users need:

```ts
semaglutide:  { …, strengths: [0.25, 0.5, 1.0, 1.7, 2.4], strengthUnit: "mg", cadence: "weekly" },
tirzepatide:  { …, strengths: [2.5, 5, 7.5, 10, 12.5, 15], strengthUnit: "mg" },
retatrutide:  { …, strengths: [1, 2, 4, 8, 12], strengthUnit: "mg" },
```

This is reference data, not medical advice — it belongs behind the same disclaimer
`MedicationDisclaimer.tsx` already renders, and `glp1.ts` already carries that framing in its
header comment ("estimate for visualization and education only"). **Retatrutide's ladder in
particular is community-convention, not a label** — mark it as such in the data, and have the UI
say so.
...
**Phase 1 — Local autofill, no network.** Combobox over Tiers 1 + 2 with the always-present custom
row; curated `strengths` ladders; dosage chips + Custom. Web and mobile. *Ships the retatrutide
example end to end with zero privacy surface and zero external dependency.* This is the phase that
solves the actual user's problem, and it should ship alone before Tier 3 is written.

**Phase 2 — RxTerms behind the opt-in.** `integrations/rxterms/RxTermsService.ts`, a v2 route,
the settings toggle, server cache, graceful degradation. *Ships the testosterone example.*

**Phase 3 — Quality.** `approximateTerm` typo fallback; brand→generic display ("Wegovy → semaglutide");
recency weighting so a user's own drugs outrank the catalog.

**Phase 4 — Optional enrichment.** Filtered openFDA for NDC/labeler on an already-chosen drug;
...
1. **Curated catalog scope.** Ship Phase 1 with GLP-1s only (6 profiles, already written), or
   broaden to TRT/peptides — testosterone esters, HCG, BPC-157? Broader helps this app's users but
   is hand-maintained data with no upstream source of truth.
2. **Retatrutide ladder provenance.** Community convention, not a label. Include it with an
   explicit "not label-derived" marker, or leave retatrutide strengths empty and force Custom?
3. **Opt-in default.** I recommend off, matching the migration's comment. Counter-argument: a
   self-hosted user who added the module probably wants the catalog, and an off-by-default toggle
   they never find makes Phase 2 look broken.
