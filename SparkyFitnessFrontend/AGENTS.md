...
- Beyond-use date: `BUD_WINDOW_DAYS` (28) is a _suggestion_, prefilled from the opened date and overwritable, with the assumption it makes spelled out next to the field. 28 days is the bacteriostatic-water figure — the benzyl alcohol is what buys the month — and is badly wrong for sterile preservative-free water, which the reconstitution record does not capture. Once a BUD is the user's (typed here, or already stored on the row) `budTouched` stops the opened date from recomputing it; do not "fix" that by re-deriving on every change.
