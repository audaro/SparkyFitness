...
const ExerciseSearchScreen: React.FC<ExerciseSearchScreenProps> = ({
  ...
  const [resolvingExerciseId, setResolvingExerciseId] = useState<string | null>(
    null
  );
  ...
  const handleSelectSuggested = useCallback(
    async (item: AlternativeExercise) => {
      if (selectionInFlightRef.current) return;
      selectionInFlightRef.current = true;
  ...
  const renderSuggestedRow = useCallback(
    (item: AlternativeExercise) => {
      const image = item.images[0] ?? null;
      const subtitle =
