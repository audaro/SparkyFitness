...
describe('classifyLoginError', () => {
  test('429 is rate limiting, whatever the body says', () => {
    ...
    expect(
      classifyLoginError(new LoginError('too many requests; expired', 429))
    ).toBe('rate-limited');
  });

  test('a bad verification code is called out specifically', () => {
    expect(classifyLoginError(new LoginError('Invalid code', 400))).toBe(
      'invalid-code'
    );
  });
  ...
  describe('a stale two-factor session', () => {
    ...
    test('is still recognized from the message when no code field arrives', () => {
      ...
      expect(
        classifyLoginError(new LoginError(STALE_TWO_FACTOR_CODE, 401))
      ).toBe('stale-two-factor-session');
    });

    test('is recognized from the spelled-out message, case-insensitively', () => {
      expect(
        classifyLoginError(new LoginError('Invalid Two Factor Cookie', 401))
      ).toBe('stale-two-factor-session');
    });
  ...
  describe('errors that never reached the server', () => {
    ...
    test('do not classify as a stale session even when the message says expired', () => {
      expect(classifyLoginError(new LoginError('Your passkey expired'))).toBe(
        'auth-failed'
      );
    });
    ...
    test('are still rate-limit-free and code-free — just an auth failure', () => {
      expect(
        classifyLoginError(
          new LoginError('A secure (HTTPS) server URL is required to sign in.')
        )
      ).toBe('auth-failed');
    });
    ...
    test('an invalid code is still named before the no-status guard', () => {
      ...
      expect(classifyLoginError(new LoginError('invalid code'))).toBe(
        'invalid-code'
      );
    });
  });
  ...
  test('a LoginError nothing matches is a generic auth failure', () => {
    expect(
      classifyLoginError(new LoginError('Sign-in failed: 500 - boom', 500))
    ).toBe('auth-failed');
  });
  ...
  test('anything that is not a LoginError is unknown, not an auth failure', () => {
    ...
    expect(classifyLoginError(new Error('INVALID_TWO_FACTOR_COOKIE'))).toBe(
      'unknown'
    );
