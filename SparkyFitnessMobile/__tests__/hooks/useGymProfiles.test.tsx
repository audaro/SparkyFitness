...
const mockFetch = fetchGymProfiles as jest.MockedFunction<
  typeof fetchGymProfiles
>;
const mockCreate = createGymProfile as jest.MockedFunction<
  typeof createGymProfile
>;
const mockUpdate = updateGymProfile as jest.MockedFunction<
  typeof updateGymProfile
>;
const mockDelete = deleteGymProfile as jest.MockedFunction<
  typeof deleteGymProfile
>;
const mockActivate = activateGymProfile as jest.MockedFunction<
  typeof activateGymProfile
>;
...
function renderBoth() {
  ...
  const list = renderHook(() => useGymProfiles(), {
...
describe('useGymProfileMutations', () => {
  ...
  test('activating refetches rather than patching the cache', async () => {
    ...
    await waitFor(() =>
      expect(list.result.current.activeProfile?.id).toBe('p-1')
    );
    ...
    await waitFor(() =>
      expect(list.result.current.activeProfile?.id).toBe('p-2')
    );
    expect(
      (client.getQueryData(gymProfilesQueryKey) as GymProfile[]).filter(
        (p) => p.is_active
      )
    ).toHaveLength(1);
  });
  ...
  test('isSaving covers create and update but not activate', async () => {
    ...
    act(() => {
      void mutations.result.current.createProfileAsync({
        name: 'X',
        equipment: [],
      });
    });
  ...
  test('isActivating tracks the activate mutation alone', async () => {
    ...
    await waitFor(() =>
      expect(mutations.result.current.isActivating).toBe(true)
    );
    expect(mutations.result.current.isSaving).toBe(false);
...
describe('mutation failures', () => {
  const expectToast = (text1: string, text2: string) =>
    waitFor(() =>
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'error', text1, text2 })
      )
    );
  ...
  test('a 409 on create is reported as a duplicate name, which the user can fix', async () => {
    ...
    await act(async () => {
      await expect(
        mutations.result.current.createProfileAsync({
          name: 'Home Gym',
          equipment: [],
        })
      ).rejects.toThrow();
  ...
  test('a non-409 whose body merely contains "409" is not a duplicate name', async () => {
    ...
    mockCreate.mockRejectedValue(
      serverError(500, 'profile 409abc-dead-beef could not be written')
    );
  ...
  test('each mutation names the operation that failed', async () => {
    ...
    await act(async () => {
      await expect(
        mutations.result.current.updateProfileAsync({
          id: 'p-1',
          payload: { name: 'X' },
        })
      ).rejects.toThrow();
      await expect(
        mutations.result.current.deleteProfileAsync('p-1')
      ).rejects.toThrow();
      await expect(
        mutations.result.current.activateProfileAsync('p-1')
      ).rejects.toThrow();
    });
