...
const mockUpdate = updateFoodEntryMeal as jest.MockedFunction<
  typeof updateFoodEntryMeal
>;
...
const expectMessage = (text2: string) =>
  waitFor(() =>
    expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ text2 }))
  );
...
describe('useUpdateFoodEntryMeal', () => {
  ...
  test('a non-403 whose body merely contains "403" is not a permission problem', async () => {
    ...
    mockUpdate.mockRejectedValue(
      apiError(500, 'meal 403abc-dead-beef is corrupt')
    );
