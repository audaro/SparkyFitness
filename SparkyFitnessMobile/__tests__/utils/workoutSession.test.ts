...
describe('workoutSession', () => {
  ...
  describe('live-start payload builders', () => {
    ...
    describe('buildSingleExerciseStartPayload', () => {
      ...
      it('parses under the request schema', () => {
        const [exercise] = buildSingleExerciseStartPayload({
          id: EX_A,
          modality: null,
          category: null,
        });
  ...
  describe('backfillPlannedSetValues', () => {
    ...
    it('fills skipped sets from the plan and leaves completed sets alone', () => {
      ...
      expect(result.exercises[0].sets[1]).toMatchObject({
        reps: 10,
        weight: 40,
      });
    });
    ...
    it('returns the same session object when nothing needs filling', () => {
      const input = session();
      expect(backfillPlannedSetValues(input, { '1': 1, '2': 2 }, planned)).toBe(
        input
      );
...
describe('workout recommendations', () => {
  ...
  describe('buildRecommendationStartPayload', () => {
      ...
      const payload = makePayload({
        exercises: [
          makeRecommendedExercise({
            sets: [
              makeRecommendedSet({
                set_type: 'Backoff' as RecommendationSet['set_type'],
              }),
            ],
          }),
        ],
    ...
    it('keeps warm-ups first and renumbers sets from 1', () => {
      ...
      const sets = buildRecommendationStartPayload(
        orderedRecommendationExercises(payload)
      )[0].sets;
      expect(sets.map((set) => set.set_number)).toEqual([1, 2, 3, 4]);
      expect(sets.slice(0, 2).map((set) => set.set_type)).toEqual([
        'warmup',
        'warmup',
      ]);
    ...
    it('passes metric values through without conversion', () => {
      const payload = makePayload({
        exercises: [
          makeRecommendedExercise({
            sets: [
              makeRecommendedSet({ weight: 82.5, reps: 10, rest_time: 150 }),
            ],
          }),
        ],
      });
    ...
    it('orders exercises by sort_order and renumbers them from 0', () => {
      ...
      expect(
        buildRecommendationStartPayload(
          orderedRecommendationExercises(payload)
        ).map((exercise) => [exercise.exercise_id, exercise.sort_order])
      ).toEqual([
        [EX_A, 0],
        [EX_B, 1],
      ]);
  ...
  describe('buildRecommendationDraftExercises', () => {
    ...
    const clientIdsFor = (payload: WorkoutRecommendationPayload) =>
      orderedRecommendationExercises(payload).map((exercise, index) => ({
        ...
        setClientIds: exercise.sets.map(
          (_set, setIndex) => `set-${index}-${setIndex}`
        ),
      }));
    ...
    it('gates the cardio measures exactly as starting the workout does', () => {
      ...
      expect(build(payload)[0].sets![0]).toMatchObject({
        distance: '5',
        restTime: 0,
      });
    });
    ...
    it('drops a distance the modality cannot mean', () => {
      const payload = makePayload({
        exercises: [
          makeRecommendedExercise({
            sets: [makeRecommendedSet({ distance: 5 })],
          }),
        ],
      });
    ...
    it('maps in prescribed order, not array order', () => {
      ...
      expect(
        build(payload).map((draft) => [draft.exerciseId, draft.clientId])
      ).toEqual([
        [EX_A, 'ex-0'],
        [EX_B, 'ex-1'],
      ]);
  ...
  describe('formatRecommendedSets', () => {
    ...
    it('renders cardio as one duration + distance block in the display unit', () => {
      ...
      expect(formatRecommendedSets(exercise, 'kg', 'miles')).toBe(
        '30:00 · 3.23 mi'
      );
    });
