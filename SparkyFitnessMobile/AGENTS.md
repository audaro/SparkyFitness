...
- Weekly set targets (working sets per training group against a weekly goal) live in `WeeklySetTargetsScreen`, reached from `WorkoutSettingsScreen`, backed by `services/api/weeklySetTargetsApi.ts` + `hooks/useWeeklySetTargets.ts`. The hexagon ring is `components/HexagonProgressRing.tsx` (Skia, one arc per group, static paths — the React Compiler rejects manual memos in this screen, so do not reintroduce `useMemo` there). Editing sends only the changed group: the server merges a partial map, and resending all four would clobber an edit made elsewhere. The mutation writes the server's recomputed response straight into the cache rather than invalidating.
