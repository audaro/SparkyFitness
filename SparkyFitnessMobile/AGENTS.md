...
- Recommendation-family changes ("Up Next", recovery, muscle targeting, on-demand, gym profiles): rerun `UpNextScreen`, `UpNextCard`, `PickMusclesScreen`, `OnDemandWorkoutsScreen`, `ExerciseHomeScreen`, `GymProfilesScreen`, `MuscleRecoveryStrip`, `useMuscleRecovery`, `useWeeklySetTargets`, `workoutRecommendationsApi`, `muscleTiles`, and `workoutSession` tests. A new root-stack screen here also has to satisfy `__tests__/navigation/nativeHeaderContract.test.ts`. Note what does **not** have a suite of its own: `hooks/useWorkoutRecommendation.ts`, `hooks/useGymProfiles.ts` and `utils/workoutSupersets.ts` are covered only through the screens that use them, so a change to one of those is only as tested as `UpNextScreen` and `ActiveWorkoutScreen` happen to make it.
