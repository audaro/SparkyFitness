...
- Weekly set targets (working sets per training group against a weekly goal) live in `WeeklySetTargetsScreen`, reached from the Exercise tab's "This week" card and from its Setup section (the card hides itself when the read came back with nothing, so the row is what keeps the screen reachable), backed by `services/api/weeklySetTargetsApi.ts` + `hooks/useWeeklySetTargets.ts`. The hexagon ring is `components/HexagonProgressRing.tsx` (Skia, one arc per group, static paths — the React Compiler rejects manual memos in this screen, so do not reintroduce `useMemo` there). It renders at two sizes on two screens, so the group colours come from `hooks/useWeeklySetGroupColors.ts` and set counts from `formatSetCount` (`utils/workoutSession.ts`) — counts are fractional because a secondary mover is half a set, so never round them. Editing sends only the changed group: the server merges a partial map, and resending all four would clobber an edit made elsewhere. The mutation writes the server's recomputed response straight into the cache rather than invalidating.
- Per-muscle recovery is `components/MuscleRecoveryStrip.tsx` on the Exercise tab, backed by `fetchMuscleRecovery` (`services/api/workoutRecommendationsApi.ts`) + `hooks/useMuscleRecovery.ts` against `GET /api/workout-recommendations/recovery`. The wire's `freshness` is **0.0–1.0, not a percentage**; the hook's `select` derives `percent` and a coarse `tone` once per fetch through `utils/muscleRecoveryDisplay.ts`, so nothing downstream multiplies by 100 again. The response is a complete vector over all 17 canonical muscles, already sorted freshest-first — render it in the order it arrives rather than re-sorting, so the strip agrees with the muscles the generator would pick. The strip hides on empty data, never on `isError` (it refetches on focus, so the error flag is true over cached data whenever the user is offline).
- `UpNextScreen`'s "Swap workout" button opens the **swap sheet**: an `ActionSheet` of every way to get a different workout — Pick Muscles, Saved Workouts, Create From Scratch, and a Refresh row that regenerates the same targets (`generate({swap: true})`, the screen's only whole-workout swap path). "On Demand" is not there yet. `renderContent()` only reaches that button once a workout exists; the loading/error/empty branches render a `StatusView`, which takes exactly one action and has already spent it on Generate/Retry — so in those states the sheet moves to a neutral "New" header item instead, and exactly one of the two entry points is ever on screen. Neither is `role: 'primary'`, so the screen still declares no accent header action.
- Muscle targeting for the next generated workout is `PickMusclesScreen`, reached from the swap sheet's "Pick Muscles" row. It is one root-stack screen with two modes (`GymProfilesScreen`'s list/editor shape), a split list and a muscle grid of `components/MuscleTile.tsx`. **Splits are resolved to canonical muscles on the client** — `MUSCLE_SPLITS` / `MUSCLE_SPLIT_MEMBERS` from `@workspace/shared` — and the generate request carries the muscles, never a split name. "Recovered muscles" is the *absence* of a constraint and must **omit** `target_muscles`: the field is `.min(1)`, so `[]` is a 400, and omitting it is a different request from naming every muscle. The grid's display grouping lives in `constants/muscleTiles.ts` (Back is one tile covering `lats` + `middle back`); it is UI only, it partitions all 17 canonical muscles, and `__tests__/constants/muscleTiles.test.ts` asserts that. Tiles read the `percent` `useMuscleRecovery` already derived — a tile covering two muscles shows the more fatigued one — and render a labelled colour block when they are given no SVG path, which is what lets the grid ship before the anatomical art exists. Tone colours come from `hooks/useFreshnessToneColors.ts` so the grid and the recovery strip cannot disagree.
