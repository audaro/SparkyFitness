...
describe('hydrationNutritionRangeService.getHydrationNutritionRange (#2348)', () => {
  ...
  it('handles a Date object for entry_date the same as a plain string', async () => {
    // Local midnight, not `Date.UTC`: node-pg parses a `date` column as
    // `new Date(y, m - 1, d)`, and `localDateToDay` reads it back the same way.
    // A UTC-midnight fixture only agrees with that at offsets >= 0, so it
    // passes on a UTC runner and fails everywhere west of Greenwich.
    const entryDate = () => new Date(2026, 8, 1);
    measurementRepo.getWaterTotalsByDateRange.mockResolvedValue([
      { entry_date: entryDate(), total_ml: 250 },
    ]);
    reportRepo.getDailyNutritionTotalsRange.mockResolvedValue([
      {
