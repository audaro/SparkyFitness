...
describe('admin global Open Food Facts provider credentials', () => {
  ...
  it('returns editable usernames without credential secrets or encrypted storage fields', async () => {
    // Declared rather than inlined: the repository's row type carries an index
    // signature that the spread in its return drops, so a fresh literal trips
    // excess-property checking on the columns the route is meant to strip.
    const globalProviderRow = {
      id: PROVIDER_ID,
      provider_name: 'Global OFF',
      provider_type: 'openfoodfacts',
      is_active: true,
    ...
    ).mockResolvedValue([globalProviderRow]);
