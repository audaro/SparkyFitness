...
- Muscle recovery / workout recommendation issue (the freshness scores the Fitbod-style generator picks muscles from):
  inspect `routes/workoutRecommendationRoutes.ts`, `services/workoutRecommendationService.ts`, `models/workoutRecommendationRepository.ts`, and the pure scoring in `../shared/src/utils/muscleRecovery.ts`. Nothing is logged for this — fatigue is derived from the `exercise_entries` muscle **snapshots** joined to their set counts, with an exponential decay. The scoring function reads no clock and no database (the caller passes `today` from `todayInZone(tz)`), because "Up Next" has to be stable across app opens; keep it that way
