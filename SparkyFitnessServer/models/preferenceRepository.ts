  ...
  try {
    const result = await client.query(
      `UPDATE user_preferences SET
        ...
        medication_catalog_lookup_enabled = COALESCE($54, medication_catalog_lookup_enabled),
        updated_at = now()
      WHERE user_id = $28
      RETURNING *`,
      [
  ...
  try {
    const result = await client.query(
     ...
     ON CONFLICT (user_id) DO UPDATE SET
       ...
       medication_catalog_lookup_enabled = COALESCE($54, user_preferences.medication_catalog_lookup_enabled),
       updated_at = now()
     RETURNING *`,
      [
