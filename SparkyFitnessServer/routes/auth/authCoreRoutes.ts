...
const mfaFactorsRateLimit = (() => {
  ...
  return (req: Request<RouteParams>, res: Response, next: NextFunction) => {
    // `getClientIp` rather than `req.ip`: behind more than one proxy `req.ip`
    // lands on an address every visitor shares, which would put the whole
    // internet in one bucket. It always returns a string, so there is no
    // undefined case left to fold into a shared bucket here.
